...
public class HudElementHealthMountExtended extends HudElement {
	...
	public void drawElement(DrawContext gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		drawCustomBar(gui, posX, posY, 88, 8, (double) health / (double) healthMax * 100.0D, -1, -1, this.settings.getIntValue(Settings.color_health), offsetColorPercent(this.settings.getIntValue(Settings.color_health), OFFSET_PERCENT));
		...
		if (this.settings.getBoolValue(Settings.show_numbers_health)) {
			...
			gui.drawCenteredTextWithShadow( this.mc.textRenderer, stringHealth, posX * 2 + 88, posY * 2 + 4, -1);
			ms.scale(2f, 2f, 2f);
		}
	}
...
//		if (this.settings.getBoolValue(Settings.show_numbers_health)) {
//			ms.scale(0.5f, 0.5f, 0.5f);
//			ms.drawCenteredTextWithShadow( this.mc.textRenderer, stringHealth, posX * 2 + 88, posY * 2 + 4, -1);
//			ms.scale(2f, 2f, 2f);
//		}
//	}
