	...
	public void drawElement(DrawContext gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		if (stats.isNotFull() && this.settings.getBoolValue(Settings.show_hunger_preview)) {
			...
			if (value > 0) {
				...
				drawCustomBar(gui, posX, posY, width, 8, bonusHunger / (double) staminaMax * 100.0D, -1, -1, colorPreview, offsetColorPercent(colorPreview, OFFSET_PERCENT), false);
			}
		}

		if (this.mc.player.hasStatusEffect(StatusEffects.HUNGER)) {
			drawCustomBar(gui, posX, posY, width, 8, stamina / (double) staminaMax * 100.0D, -1, -1, this.settings.getIntValue(Settings.color_hunger), offsetColorPercent(this.settings.getIntValue(Settings.color_hunger), OFFSET_PERCENT), false);
		} else {
			drawCustomBar(gui, posX, posY, width, 8, stamina / (double) staminaMax * 100.0D, -1, -1, this.settings.getIntValue(Settings.color_food), offsetColorPercent(this.settings.getIntValue(Settings.color_food), OFFSET_PERCENT), false);
		}
		...
		if (this.settings.getBoolValue(Settings.show_numbers_food)) {
			...
			gui.drawCenteredTextWithShadow( this.mc.textRenderer, staminaString, posX * 2 + width, posY * 2 + 4, -1);
			ms.scale(2f, 2f, 2f);
		}
	}
