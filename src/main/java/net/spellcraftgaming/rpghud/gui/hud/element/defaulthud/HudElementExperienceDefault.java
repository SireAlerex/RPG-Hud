...
public class HudElementExperienceDefault extends HudElement {
	...
	public void drawElement(DrawContext gui, Identifier text, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		drawCustomBar(gui, posX, scaledHeight - 10 + posY, scaledWidth, 10, exp * full, this.settings.getIntValue(Settings.color_experience), offsetColorPercent(this.settings.getIntValue(Settings.color_experience), 25));
		...
		if (this.settings.getBoolValue(Settings.show_numbers_experience))
			gui.drawCenteredTextWithShadow(this.mc.textRenderer, stringExp, var7 + posX, scaledHeight - 9 + posY, -1);
	}
