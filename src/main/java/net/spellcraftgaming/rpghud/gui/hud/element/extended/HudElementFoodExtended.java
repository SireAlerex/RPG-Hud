	...
	public void drawElement(DrawContext gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		if (stats.isNotFull() && this.settings.getBoolValue(Settings.show_hunger_preview)) {
			...
			if (value > 0) {
				...
				drawCustomBar(gui, posX, posY, 110, 12, bonusHunger / (double) staminaMax * 100.0D, -1, -1, colorPreview, offsetColorPercent(colorPreview, OFFSET_PERCENT));
			}
		}

		if (this.mc.player.hasStatusEffect(StatusEffects.HUNGER)) {
			drawCustomBar(gui, posX, posY, 110, 12, stamina / (double) staminaMax * 100.0D, -1, -1, this.settings.getIntValue(Settings.color_hunger), offsetColorPercent(this.settings.getIntValue(Settings.color_hunger), OFFSET_PERCENT));
		} else {
			drawCustomBar(gui, posX, posY, 110, 12, stamina / (double) staminaMax * 100.0D, -1, -1, this.settings.getIntValue(Settings.color_food), offsetColorPercent(this.settings.getIntValue(Settings.color_food), OFFSET_PERCENT));
		}
		...
		if (this.settings.getBoolValue(Settings.show_numbers_food))
			gui.drawCenteredTextWithShadow(this.mc.textRenderer, staminaString, posX + 55, posY + 2, -1);
	}
...
//		String staminaString = this.settings.getBoolValue(Settings.hunger_percentage) ? (int) Math.floor((double) stamina / (double) staminaMax * 100) + "%" : stamina + "/" + staminaMax;
//		if (this.settings.getBoolValue(Settings.show_numbers_food))
//			ms.drawCenteredTextWithShadow( this.mc.textRenderer, staminaString, posX + 55, posY + 2, -1);
//	}
