...
//		bind(INTERFACE);
		...
		if (absorption > 1)
			gui.drawTexture(INTERFACE, posX, posY, 0, 88, (int) (110.0D * ((double) (health + absorption) / (double) (healthMax + absorption))), 12);
		if (this.mc.player.hasStatusEffect(StatusEffects.POISON)) {
			gui.drawTexture(INTERFACE, posX, posY, 141, 160, (int) (110.0D * ((double) health / (double) (healthMax + absorption))), 12);
		} else if (this.mc.player.hasStatusEffect(StatusEffects.WITHER)) {
			gui.drawTexture(INTERFACE, posX, posY, 34, 244, (int) (110.0D * ((double) health / (double) (healthMax + absorption))), 12);
		} else {
			gui.drawTexture(INTERFACE, posX, posY, 0, 100, (int) (110.0D * ((double) health / (double) (healthMax + absorption))), 12);
		}
