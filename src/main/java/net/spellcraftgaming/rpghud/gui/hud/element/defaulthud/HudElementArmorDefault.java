...
public class HudElementArmorDefault extends HudElement {
	...
	@Override
	public void drawElement(DrawContext gui, Identifier texture, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		for (int i = 1; level > 0 && i < 20; i += 2) {
			if (i < level) {
				gui.drawTexture(texture, left + 48, top - 2, 34, 9, 9, 9);
			} else if (i == level) {
				gui.drawTexture(texture, left + 48, top - 2, 25, 9, 9, 9);
			} else if (i > level) {
				gui.drawTexture(texture, left + 48, top - 2, 16, 9, 9, 9);
			}
			left += 8;
		}
	}
