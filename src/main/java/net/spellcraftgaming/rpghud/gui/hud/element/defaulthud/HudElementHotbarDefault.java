...
import net.spellcraftgaming.rpghud.gui.Textures;
...
public class HudElementHotbarDefault extends HudElement {
    ...
    public void drawElement(DrawContext gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
        if(this.mc.interactionManager.getCurrentGameMode() == GameMode.SPECTATOR)
            ...
            float f = zLevel;
            zLevel = -90.0F;
            gui.drawTexture(WIDGETS_TEX_PATH, i - 91, scaledHeight - 22 + posY, 0, 0, 182, 22);
            gui.drawTexture(WIDGETS_TEX_PATH, i - 91 - 1 + this.mc.player.getInventory().selectedSlot * 20, scaledHeight - 22 + posY - 1, 0, 22, 24, 22);
            if(!itemstack.isEmpty())
                if(arm == Arm.LEFT)
                    gui.drawTexture(WIDGETS_TEX_PATH, i - 91 - 29, scaledHeight - 23 + posY, 24, 22, 29, 24);
                else
                    gui.drawTexture(WIDGETS_TEX_PATH, i + 91, scaledHeight - 23 + posY, 53, 22, 29, 24);
            ...
            for(int l = 0; l < 9; ++l) {
                ...
                this.renderHotbarItem(gui, i1, j1, partialTicks, this.mc.player, this.mc.player.getInventory().main.get(l));
            }

            if(!itemstack.isEmpty()) {
                int l1 = scaledHeight - 16 - 3 + posY;
                if(arm == Arm.LEFT)
                    this.renderHotbarItem(gui, i - 91 - 26, l1, partialTicks, this.mc.player, itemstack);
                else
                    this.renderHotbarItem(gui, i + 91 + 10, l1, partialTicks, this.mc.player, itemstack);
            }
            ...
            if(this.mc.options.getAttackIndicator().getValue() == AttackIndicator.HOTBAR) {
                ...
                if(f1 < 1.0F) {
                    ...
                    int k1 = (int) (f1 * 19.0F);
                    RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
                    gui.drawTexture(Textures.ICONS, j2, i2, 0, 94, 18, 18);
                    gui.drawTexture(Textures.ICONS, j2, i2 + 18 - k1, 18, 112 - k1, 18, k1);
                }
            }
