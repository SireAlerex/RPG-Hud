	...
	public void drawElement(DrawContext gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		if (stats.isNotFull() && this.settings.getBoolValue(Settings.show_hunger_preview)) {
			...
			if (value > 0) {
				...
				drawCustomBar(gui, posX, height - 26, 200, 10, bonusHunger / (double) staminaMax * 100.0D, -1, -1, colorPreview, offsetColorPercent(colorPreview, OFFSET_PERCENT));
			}
		}

		if (this.mc.player.hasStatusEffect(StatusEffects.HUNGER)) {
			drawCustomBar(gui, posX, height - 26, 200, 10, stamina / (double) staminaMax * 100.0D, -1, -1, this.settings.getIntValue(Settings.color_hunger), offsetColorPercent(this.settings.getIntValue(Settings.color_hunger), OFFSET_PERCENT));
		} else {
			drawCustomBar(gui, posX, height - 26, 200, 10, stamina / (double) staminaMax * 100.0D, -1, -1, this.settings.getIntValue(Settings.color_food), offsetColorPercent(this.settings.getIntValue(Settings.color_food), OFFSET_PERCENT));
		}
		...
		if (this.settings.getBoolValue(Settings.show_numbers_food))
			gui.drawCenteredTextWithShadow( this.mc.textRenderer, staminaString, posX + 100, height - 25, -1);
	}
