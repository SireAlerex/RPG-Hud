...
public class HudElementExperienceSimple extends HudElement {
	...
	public void drawElement(DrawContext gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		drawCustomBar(gui, posX, posY, width, 8, exp * full, 0xA0000000, 0xA0000000, this.settings.getIntValue(Settings.color_experience), offsetColorPercent(this.settings.getIntValue(Settings.color_experience), 25), 0xA0000000);
		...
		if (this.settings.getBoolValue(Settings.show_numbers_experience)) {
			...
			gui.drawCenteredTextWithShadow( this.mc.textRenderer, stringExp, posX * 2 + width, posY * 2 + 4, -1);
			ms.scale(2f, 2f, 2f);
		}
	}
