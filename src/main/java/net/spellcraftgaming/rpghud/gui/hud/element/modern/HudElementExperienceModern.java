...
public class HudElementExperienceModern extends HudElement {
	...
	public void drawElement(DrawContext gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		drawRect(gui, posX, scaledHeight - 7 + posY, scaledWidth, 7, 0xA0000000);
		drawRect(gui, 1 + posX, scaledHeight - 6 + posY, (int) (exp * full), 4, this.settings.getIntValue(Settings.color_experience));
		...
		if (this.settings.getBoolValue(Settings.show_numbers_experience)) {
			...
			drawRect(gui, 1 + posX, scaledHeight - 15 + posY, width2 + 4, 8, 0xA0000000);
			MatrixStack ms = gui.getMatrices();
			ms.scale(0.5f, 0.5f, 0.5f);
			gui.drawCenteredTextWithShadow( this.mc.textRenderer, stringExp, 6 + width2 + posX * 2, (scaledHeight - 12) * 2 - 1 + posY * 2, -1);
			ms.scale(2f, 2f, 2f);
		}
	}
