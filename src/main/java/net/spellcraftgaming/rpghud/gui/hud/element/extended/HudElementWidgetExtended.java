...
//		bind(INTERFACE);
		...
		gui.drawTexture(INTERFACE,posX + (this.settings.getBoolValue(Settings.render_player_face) ? 50 : 26), posY + (this.settings.getBoolValue(Settings.render_player_face) ? 4 : 0), 0, 35, 114, 44);
		if (this.mc.player.getVehicle() instanceof LivingEntity) {
			gui.drawTexture(INTERFACE,posX + (this.settings.getBoolValue(Settings.render_player_face) ? 51 : 23), posY + (this.settings.getBoolValue(Settings.render_player_face) ? 44 : 39), 164, 0, 92, 20);
		}
...
//			bind(getPlayerSkin(this.mc.player));
			...
			gui.drawTexture(player_skin,posX * 2 + 34 + facePosX * 2, posY * 2 + 34 + facePosY * 2, 32, 32, 32, 32);
			gui.drawTexture(player_skin,posX * 2 + 34 + facePosX * 2, posY * 2 + 34 + facePosY * 2, 160, 32, 32, 32);
			ms.scale(2f, 2f, 2f);
//			bind(AbstractParentElement.GUI_ICONS_TEXTURE);
		} else {
			gui.drawTexture(INTERFACE,posX, 3, 214, 20, 26, 38);
//			bind(AbstractParentElement.GUI_ICONS_TEXTURE);
		}
	}
