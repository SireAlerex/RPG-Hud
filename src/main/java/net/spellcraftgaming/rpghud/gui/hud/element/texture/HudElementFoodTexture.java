...
//		bind(INTERFACE);
		...
		if (stats.isNotFull() && this.settings.getBoolValue(Settings.show_hunger_preview)) {
			...
			if (value > 0) {
				...
				gui.drawTexture(INTERFACE, posX, posY, 141, 148, (int) (110.0D * (bonusHunger / (double) staminaMax)), 12);
			}
		}

		if (this.mc.player.hasStatusEffect(StatusEffects.HUNGER)) {
			gui.drawTexture(INTERFACE, posX, posY, 141, 136, (int) (110.0D * (stamina / (double) staminaMax)), 12);
		} else {
			gui.drawTexture(INTERFACE, posX, posY, 110, 100, (int) (110.0D * (stamina / (double) staminaMax)), 12);
		}
